...
const OrderRequestDetailsPage = ({ role }) => {
  const { id } = useParams();
  ...
  const { 
    ...
    error,
    refetch 
  } = useGetOrderRequestByIdQuery(id);
  ...
  const canCancel = role === 'stockist' && orderRequest?.status === 'pending';
  const canApproveReject = role === 'admin' && orderRequest?.status === 'pending';
